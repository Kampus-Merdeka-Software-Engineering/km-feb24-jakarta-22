...
async function loadData() {
    try {
        const response1 = await fetch('../JSDataCategory/RevenueSupreme.json');
        if (!response1.ok) throw new Error('Failed to load RevenueSupreme.json');
        ...
        const response2 = await fetch('../JSDataCategory/QuantitySupreme.json');
        if (!response2.ok) throw new Error('Failed to load QuantitySupreme.json');
