...
async function loadData() {
    try {
        const response1 = await fetch('../JSDataCategory/RevenueVeggie.json');
        if (!response1.ok) throw new Error('Failed to load RevenueVeggie.json');
        ...
        const response2 = await fetch('../JSDataCategory/QuantityVeggie.json');
        if (!response2.ok) throw new Error('Failed to load QuantityVeggie.json');
